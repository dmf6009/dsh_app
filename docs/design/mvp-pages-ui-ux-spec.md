...
## 6. 页面四：Settings（Models / DSH / Permissions / Plugins）

> v1.1 增补：在 v1.0 三个 Tab（§37 原文）之外新增第四个 **Plugins** 页签，
> 承载 dsh「万物皆可插」的桌面能力，规范见 §11。
...
- **Models**（§17；v1.2 按 dsh 原生 schema 重做）：Provider 列表从 `~/.dsh/settings.yaml` 的 dsh 原生插件段加载——`llm-pi-ai.providers.*`（多 Provider，含 openrouter/google 等无 baseURL 的内置端点 Provider）与 `llm-deepseek`（密钥 env 默认 `DEEPSEEK_API_KEY`）；模型为富对象条目（id / 显示名 / 上下文窗口 / 最大输出），可增删改参数，磁盘上已有而界面未列出的字段（多模态、图片预算等）写回时原样保留；API Key 按 dsh 原生约定写入 `~/.dsh/.credentials.yaml` 的 `refs.<apiKeyEnv>`（掩码显示，不明文回看，文件保持 0600）；支持设置默认模型（写入 `agent-default-model`）。**Workspace 顶栏模型选择器选择即写入 `agent-default-model` 段**——headless 每个 run 启动时读取该段，下一次运行即生效（web / CLI 同步生效），下拉初始值亦显示该段的真实配置。写回使用 YAML CST（parseDocument）只改动自有段落——注释、格式与未知段落（ui-onboarding 等）原样保留；settings.yaml 解析失败时拒绝保存并提示修复，绝不覆盖。
...
## 11. 增补设计（v1.1）：Settings → Plugins 页签（dsh「万物皆可插」）

> **定位**：dsh 最核心的架构亮点是「Everything is a Plugin」——npm 包即插件，
> 通过 profile 的插件束（bundle）层级栈扩展模型适配、工具、UI 与工作流。
> 本页签把这一能力纳入桌面端：管理**运行 profile**（desktop 适配器驱动
> Agent 执行所用的 profile，默认 `headless`）的插件。本节为 v1.0 需求基线
> 之外的产品增量，已获产品负责人批准。
...
```text
┌──────────────────────────────────────────────────────────┐
│ 运行 profile 的插件  <headless>                           │
│ 「dsh 的核心是万物皆可插：npm 包即插件……变更对下一次      │
│   Agent 运行生效。」                                      │
│                                                          │
│ ┌────────────────────────────────────────────────────┐   │
│ │ @deepseek-ai/dsh-base                              │   │
│ │ v0.1.1-rc.2  [插件束][核心]        [卸载](禁用)     │   │
│ ├────────────────────────────────────────────────────┤   │
...
│                                                          │
│ 安装插件                                                  │
│   npm 包名（可含版本范围）: [ @scope/dsh-plugin@^1.0.0 ]  │
│   [安装]  [刷新列表]                                      │
│   安装/卸载通过官方 `dsh plugin` 命令执行，声明           │
│   dsh.bundle 的包会自动加入启动栈。                        │
└──────────────────────────────────────────────────────────┘
```

### 11.2 数据与语义（实现口径）

- **列表数据源**：直接读 profile 清单 `~/.dsh/profiles/<profile>/package.json`
  （安装状态的事实源）。`dsh.profile.bundles` = 插件束（有序启动栈，
  `[插件束]` 徽标）；其余 `dependencies` = 普通依赖（`[依赖]` 徽标）。
  版本号从共享存储 `~/.dsh/profiles/node_modules/<pkg>/package.json` 解析。
- **安装/卸载**：一律走官方 `dsh plugin --profile <profile> add|remove <pkg>`
  （内部转发 pnpm，并按安装状态自动协调插件束栈——声明 `dsh.bundle` 的包
  自动加入，卸载自动移出）。UI 不直接改写 profile 文件。
- **保护规则**：`@deepseek-ai/dsh-base` 为 dsh 核心组件，卸载按钮禁用
  （`[核心]` 徽标 + tooltip），主进程同步强制拒绝（双层防护）。
- **生效时机**：桌面 runtime 每个 run 独立拉起 dsh，因此插件变更**对下一次
  Agent 运行自然生效**，无需重启应用（文案明示，避免「为什么没生效」困惑）。
- **安全**：包名/版本范围在主进程校验（拒绝 `-` 开头的 flag 注入与空白字符），
  拼接 argv 执行，不经 shell。

### 11.3 交互流程

1. 进入 Plugins 页签 → 读取列表（加载中骨架）；profile 未初始化时展示
   信息条「首次安装插件时会自动创建 profile 目录」。
2. 安装：输入 npm 包名（可含版本范围）→ 安装 → 成功后列表刷新、输入清空、
   轻提示「已安装 X，对下一次 Agent 运行生效」；失败停留当前页显示错误
   （含 dsh/pnpm 输出尾部）。
3. 卸载：confirm 二次确认 → 卸载 → 列表刷新；核心组件不可卸载。
4. 顶部「重新检测 DSH」（DSH 页签）变更 dsh 路径后，插件操作自动跟随
   新的 dsh CLI。
...
| 状态 | 触发条件 | UI 表现 |
| --- | --- | --- |
| 空态 | profile 已初始化但无插件 | 引导文案「尚未安装任何插件——把 npm 包插进来，扩展 Agent 的能力。」 |
| 加载中 | 列表读取中 | 「插件列表加载中…」占位 |
| profile 未初始化 | `~/.dsh/profiles/<profile>` 不存在 | 信息条「首次安装插件时会自动创建」+ 空态文案 |
| 清单损坏 | package.json 不可解析 | 视为空列表展示（可诊断，不崩溃） |
| 安装中 / 卸载中 | 变更命令执行中 | 按钮loading；安装输入不可重复提交 |
| 校验错误 | 包名非法 / 含 flag 注入 | 内联错误文案，不发起 CLI 调用 |
| 错误：CLI 失败 | pnpm 安装失败 / 网络问题 | 内联错误 + dsh/pnpm 输出尾部（等宽） |
| 错误：dsh 未找到 | 检测失败 | 内联错误「未找到 dsh CLI，无法管理插件（请先完成 DSH 安装/路径配置）」 |
| disabled | 核心组件 | 卸载按钮禁用 + tooltip「dsh 核心组件，不可卸载」 |

### 11.5 验收条目（增补 V17–V19）

| # | 要求 | 判定方式 |
| --- | --- | --- |
| V17 | 插件列表与 profile 清单一致：bundle/依赖徽标、版本号、核心保护三项全对（对照 `~/.dsh/profiles/<profile>/package.json`） | 走查 + 单元测试 |
| V18 | 安装/卸载走官方 `dsh plugin` 命令且 argv 精确（`plugin --profile <p> add|remove <pkg>`）；非法包名（`--flag`、空白、空串）被拒绝且不发起 CLI 调用 | 单元测试断言 argv |
| V19 | 核心组件 `@deepseek-ai/dsh-base`：渲染层卸载按钮禁用，主进程层强制拒绝，两层一致 | 渲染走查 + 单元测试 |
